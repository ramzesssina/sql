...
insert into employee(department_id, chief_id, `name`, salary) values
	...
	(4, null, "Филатова Татьяна Никитична", 131430),
	(5, null, "Федосеева Анастасия Алексеевна", 120432),
	(5, 13, "Фадеева Алиса Евгеньевна", 95058),
...
and not exists (
    ...
    and e2.chief_id is not null
);

#Zadanie 5
select department_id, sum(salary) salary
from employee group by department_id having sum(salary) = (select sum(salary) from employee group by department_id order by sum(salary) desc limit 1)
;